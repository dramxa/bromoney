...
const cancel = () => {
  emit('cancel')
  paymentsStore.cancel({ msid: msid.value })
  if(checker) clearInterval(checker)
}

onMounted(async () => {
  const res = await paymentsStore.create(props.data)
  if (res) {
    msid.value = res.msid
    timer.value = defaultTimer
    setTimer()
    checker = setInterval(async () => {
      const check = await paymentsStore.getPayment(msid.value)
      if (check.status == 'ACCEPTED') {
        router.push(`/buy/${msid.value}`)
      }
    }, 20000)
