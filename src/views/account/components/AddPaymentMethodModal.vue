<template>
  <Modal :id="'add-payment-method-modal'" :modal-open="modalOpen" :maxWidth="'531px'" @close-modal="close">
    <div class="p-10 relative">
      ...
      <div class="space-y-5">
        <div class="text-xl font-bold text-gray-800 text-center">Add payment method</div>
        <div>
          <DropdownPaymentMethod :height="'46px'" :error="errors.paymentsMethod" @change="changePaymentMethod($event)" />
        </div>
        <BasicInput
          v-model="cardNumber"
          :label="'Card number'"
    ...
    <div class="flex rounded-b-2xl items-center justify-end space-x-2.5 bg-gray-50 border-t border-gray-200 p-5">
      ...
      <button class="w-[80px] h-[38px] flex items-center justify-center bg-blue-500 text-white text-[15px] font-semibold rounded-md" @click="add">
        Add
      </button>
    </div>
  </Modal>
...
const add = async () => {
  ...
  const response = await userStore.addPaymentMethod(data)
  if (response) {
