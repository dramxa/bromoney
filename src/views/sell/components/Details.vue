  ...
  <div class="pb-5 px-[30px] space-y-5">
    ...
    <div class="space-y-1.5">
      <div class="flex items-center justify-between px-4 py-[13px]">
        <span class="text-xs font-semibold text-gray-600 uppercase">You get</span>
      ...
      <div class="flex items-center justify-between px-4 py-[13px]">
        ...
        <div class="flex items-center space-x-2">
          <img class="w-6 h-6" :src="details.psystem.imgPath" alt="">
          <span class="font-semibold text-gray-800">{{ details.psystem.name }}</span>
        </div>
      </div>
      <div class="flex items-center justify-between px-4 py-[13px]">
    ...
    <div v-if="details.status.toLowerCase() == 'cashed'" class="flex items-center space-x-6">
      <button class="h-[46px] w-1/2 grow rounded border border-gray-200 hover:border-gray-300 text-gray-500 font-semibold" @click="$emit('appeal')">Problem</button>
      <button class="h-[46px] w-1/2 grow rounded bg-blue-500 hover:bg-blue-600 text-white font-semibold" @click="$emit('confirm')">Payment received</button>
    </div>
    <div v-else-if="details.status.toLowerCase() == 'onappeal'" class="flex items-center space-x-6">
      <button class="h-[46px] w-1/2 rounded border border-gray-200 hover:border-gray-300 text-gray-500 font-semibold" @click="router.push('/order-history')">Close</button>
      <button class="h-[46px] w-1/2 grow rounded bg-blue-500 hover:bg-blue-600 text-white font-semibold" @click="$emit('confirm')">Payment received</button>
    </div>
