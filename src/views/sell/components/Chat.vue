<template>
  <div class="w-[460px] rounded-[20px] flex flex-col border border-gray-200 overflow-hidden" style="height: calc(100vh - 73px - 60px)">
    <div class="my-4 mx-[30px] bg-white flex items-center justify-center">
      <span v-if="data" class="font-semibold text-gray-800">{{ data.agent.name }}</span>
      <span v-else class="font-semibold text-gray-800">USER</span>
    ...
    <div ref="chat" class="overflow-y-auto" style="max-height: calc(100% - 83px - 24px);min-height: calc(100% - 83px - 24px);">
      <div v-if="!messages.length" class="flex flex-col items-center bg-gray-50 p-5 min-h-[100%]">
        <div v-if="data" class="text-sm text-gray-400 mb-5">{{ formatDateTime(data.acceptedAt) }}</div>
      ...
      <div class="min-h-[100%] flex flex-col justify-end bg-gray-50 p-2.5">
