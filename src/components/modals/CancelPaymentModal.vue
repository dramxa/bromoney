<template>
  <Modal :id="'cancel-payment-modal'" :modal-open="modalOpen" :maxWidth="'416px'" @close-modal="close">
    <div class="p-10 relative">
      ...
      <div class="space-y-5 flex flex-col items-center">
        <div class="text-center space-y-2.5">
          <div class="text-xl font-bold text-gray-800 text-center">Cancel the payment?</div>
          <div class="font-medium text-gray-500">If you have already transferred funds to the agent, then do not cancel this payment. Otherwise, the agent will not be able to credit them to your balance</div>
        </div>
        <div class="flex items-center justify-center space-x-4">
          <button class="h-[38px] w-[80px] flex items-center justify-center border border-gray-200 text-gray-800 text-[15px] font-semibold rounded-md shadow-sm px-3" @click="close">
            No
          </button>
          <button class="h-[38px] w-[80px] flex items-center justify-center bg-blue-500 text-white text-[15px] font-semibold rounded-md px-3" @click="cancel">
            Yes
          </button>
        </div>
      </div>
