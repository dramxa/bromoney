<template>
  <div class="relative inline-flex w-full">
    ...
    >
      <div class="text-[15px] font-medium select-none">{{activeLang.name}}</div>
      ...
      <div v-show="dropdownOpen" class="z-10 absolute right-0 translate-x-[28px] min-w-[137px] w-full top-[32px] bg-white border border-gray-200 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1">
        ...
        >
          <button
            ...
            :class="activeLang && lang.name === activeLang.name && 'text-blue-500'"
            @click="changeLang(lang)"
          >
            <div class="flex items-center">
              <span>{{lang.name}}</span>
            </div>
            <svg class="shrink-0 ml-2 fill-current text-blue-500" :class="{'invisible': !activeLang || lang.name != activeLang.name}" width="12" height="9" viewBox="0 0 12 9">
...
watch(user, (value: any) => {
  if (value && value.lang) {
    const currency = langs.value?.find(c => c.name == value.lang)
    if (currency) activeLang.value = currency
  }
})
...
const changeLang = async (lang: any) => {
  ...
  await userStore.changeLang(lang.value)
}

onMounted(() => {
  if (user.value) {
    const l = langs.value.find((el) => el.value === user.value!.lang)
    if (l) activeLang.value = l
  }
})
