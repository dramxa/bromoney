  ...
  <div class="relative inline-flex w-full">
    ...
    >
      <div v-show="dropdownOpen" class="z-10 absolute left-0 w-full bg-white border border-gray-200 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1" :class="top ? 'bottom-full translate-y-7' : 'top-full'">
        ...
        >
          <button
            v-for="(method, index) in psystems"
...
const { psystems } = storeToRefs(settingsStore)
const activeMethod = ref<any | null>(null)
const dropdownOpen = ref<boolean>(false)
const trigger = ref<HTMLElement | null>(null)
const dropdown = ref<HTMLElement | null>(null)

watch(psystems, (value) => {
  if(value?.length) changePaymentMethod(value[0])
  else activeMethod.value = null
})

const dropdownClick = () => {
  if (!psystems.value?.length) {
    emit('empty')
  } else {
